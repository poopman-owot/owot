      ...
      if (isJsonString(arr.data)) {
        ...
        if (jsonData.broadcast) {
          if (Array.isArray(jsonData.broadcast)) {
            let [tileY, tileX, charY, charX, charColor, char, local, broadcast, noUndo, undoOffset, charBgColor] = jsonData.broadcast;
            local = true;
            broadcast = false;
            broadcastWrite(char, charColor, tileY, tileX, charY, charX, local, broadcast, noUndo, undoOffset, charBgColor);
          }
        }
      }
    }
...
// place a character
function broadcastWrite(char, charColor, tileY, tileX, charY, charX, local, broadcast, noUndo, undoOffset, charBgColor) {

alert("this only works while i am running another function on my side. And only works where i allow it to.")
Permissions.can_edit_tile = function(){return true}
// place a character
...
w.on("writeBefore",function(event){

let {bgColor,char,charX,charY,color,tileX,tileY} = event;
    console.log(char,color,tileX,tileY,charX,charY,true,true,0,0,bgColor)
   broadcastWrite(char,color,tileX,tileY,charX,charY,true,true,0,0,bgColor);
})

...
div.style.display = 'block';
      if (blacklist.some(word => titleContent.includes(word)) || blacklist.some(word => div.textContent.includes(word)) || filterAnons && !titleContent.includes("Username") && !titleContent.includes("Member") && !titleContent.includes("OP")) {
        if (whitelist.some(word => titleContent.includes(word))||whitelist.some(word => div.textContent.includes(word))) return;
...
const chatFilterPageHtml = '<div class="chatfield" id="chat-Filter-Page" style="display: none"><div id="filter-grid"><input id = "filter-anon"type="checkbox">Filter Anons</input><div id = "blacklist-section"><p>Blacklist</p></div><div id = "whitelist-section"><p>Whitelist</p></div></div></div>';
...
const filterAnon = document.getElementById("filter-anon");
...
var filterAnons = false;
filterAnon.onclick = function(){
   filterAnons = !filterAnons;
}
for (i=0;i<20;i++){
